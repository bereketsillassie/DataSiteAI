...
class NASAPowerClient(BaseIntegrationClient):
    ...
    async def get_climate_data(self, lat: float, lng: float) -> dict:
        ...
        """
        cache_key = self._cache_key("climate", round(lat, 2), round(lng, 2))
        ...
        try:
            ...
            # NASA POWER returns monthly climatology — take the annual average (key "ANN")
    ...
    def _mock_climate_data(self, lat: float, lng: float) -> dict:
        ...
        humidity = 65.0 + max(0, (lng + 80.0)) * 0.3  # Higher near coast
